 ...
 */
@WebServlet("/DataAccessServlet1")
public class DataAccessServlet1 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String loginId = (String) getServletContext().getAttribute("loginId");//"soc.manager@infosys.com";//UserLoginServlet.getLoginId(request);
		String pass_Word = (String) getServletContext().getAttribute("userpassword");
		String restUrl = (String) getServletContext().getAttribute("restUrl");
		String certPath = (String) getServletContext().getAttribute("certPath");
		String certPass = (String) getServletContext().getAttribute("certPass");
		String referehInterval = (String) getServletContext().getAttribute("referehInterval");
		...
		if (action.equals("getChartData"))
		{
			...
			try{			           
		        ...
		        for(String orgName : orgNamelst){
		        	SimpleClient simpleClientForOrg = new SimpleClient(url, orgName, loginId, pass_Word,certFilepath,certPass);
		        	simpleClientForOrg.connect();
		        	//Getting the incidentData 
		        	List<IncidentDTO> incidentsDataList = simpleClientForOrg.get("incidents?handle_format=names", INC_TYPE); 
			        ...
			        for (IncidentDTO incidentch : incidentsDataList) {
			        	...
			        	if(chartDataIncidentMap.containsKey(datecreated)){
			        		...
			        		for(Object inctype : lstIncType){			        			
			        			...
			        			if(countexisting.containsKey(IncidentType)){
				...
				usersJson.put("users", getChartJson(incidentTypeLst, chartDataIncidentMap,"Incident Over Time by Type"));
				usersJson.put("networkZnChartData", getChartJson(networkZoneList, chartDataNetworkZoneMap,"Network Zone"));
		...
		else if (action.equals("getIncidentList"))
		{
			...
			try{
		        ...
		        for(String orgName : orgNamelst){
		        	...
		        	//Getting attack types information and creating a Map(attacktypeid --> attacktypeName)
		        	Map<String, Object> attackTypeMap =simpleClientForOrg.get("types/incident/fields/attack_type?handle_format=names");
		        	
		        	Map<Integer, String> attackIdLableMap = new HashMap<Integer, String>();
		        	if(attackTypeMap != null){
		        		List<LinkedHashMap> attackTypeMapLst= (List<LinkedHashMap>) attackTypeMap.get("values");
		        	...
		        	Map<String, Object> networkzoneMap =simpleClientForOrg.get("types/incident/fields/network_zone?handle_format=names");
		        	
		        	if(networkzoneMap != null){
		        		List<LinkedHashMap> networkzoneMapLst= (List<LinkedHashMap>) networkzoneMap.get("values");
		        	...
		        	Map<String, Object> categoriesMap =simpleClientForOrg.get("types/incident/fields/categories?handle_format=names");
		        	Map<Integer, String> categoriesIdLableMap = new HashMap<Integer, String>();
		        	if(categoriesMap != null){
		        		List<LinkedHashMap> categoriesMapLst= (List<LinkedHashMap>) categoriesMap.get("values");
		        	...
		        	List<IncidentDTO> incidentsData = simpleClientForOrg.get("incidents?handle_format=names", INC_TYPE); 
	...
	/**
	 * This method will create a JSON object require to generate the google chart
	 * @param properties(eg: incident type or network zone)
	 * @param DataMap (Contains eg:DateCreated-->SubMap and SubMap will contain incidentTypes --> counts)
	 * @param nameChart (This is name of the chart)
	 * @return JSONObject
	 */	
